    ...
    public class Singularity
    {
        ...
        private double[] Eigenvalues_ascending(double[,] origin_matrix)
        {
            ...
            using (StreamReader sr = new StreamReader(@"eigenvalues.txt"))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    //eigenvalues[i] = Convert.ToDouble(line);
        ...
        private void Rounding_Matrix(ref double[,] curr_matrix)
        {
            for(int i = 0; i < curr_matrix.GetLength(0); i++)
            {
                for( int j = 0; j < curr_matrix.GetLength(1); j++)
                {
                    curr_matrix[i, j] = Math.Round(curr_matrix[i, j], 5);
                }
            }
        }
        ...
        private double[,] Matrix_minus_Eignvalue(double[,] curr_matrix, double eigenvalue)
        {
            for (int i = 0; i < curr_matrix.GetLength(0); i++)
            {
        ...
        private double[,] Correct_Matrix_After_Rounding(double[,] curr_matrix)
        {
            for (int amount = 0; amount < curr_matrix.GetLength(0); amount++)
            {
                for (int i = curr_matrix.GetLength(0) - 1; i >= 1; i--)
                {
                    bool isNull = true;
                    for (int j = 0; j < curr_matrix.GetLength(1); j++)
                    {
                        if (curr_matrix[i - 1, j] != 0)
                        {
                            isNull = false;
                            break;
                        }
                    ...
                    if (isNull)
                        ChangeRows(ref curr_matrix, i, i - 1);
        ...
        private double[] Eigenvector(double[,] matrix, double eigenvalue, int size_of_current_matrix) //надо доработать
        {
            ...
            curr_matrix = Matrix_minus_Eignvalue(curr_matrix, eigenvalue);

            curr_matrix = ToTriangle(curr_matrix, eigenvalue, curr_matrix.GetLength(0));
            Rounding_Matrix(ref curr_matrix);
            curr_matrix = Correct_Matrix_After_Rounding(curr_matrix);
            ...
            {
                if (first_not_null_element == curr_matrix.GetLength(1) - 2)
                {
                    double last = curr_matrix[first_not_null_row, first_not_null_element + 1];
                    double prelast = curr_matrix[first_not_null_row, first_not_null_element];

                    vector[vector.Length - 1] = 1;

                    if(last > 0)
                        vector[vector.Length - 2] = -(last * vector[vector.Length - 1]) / prelast;
                    else
                        vector[vector.Length - 2] = (last * vector[vector.Length - 1]) / prelast;
                    vector_filled_values = 2;
                }
                else
                {
                    double sum_of_vector_values = 0;
                    for (int j = first_not_null_element + 1; j < vector.Length - 1; j++)
                    {
                        vector[j] = random.NextDouble() * 2;
        ...
        public double[,] V_transposed(double[,] origin_matrix, double[] eignvalues, double[,] AtA, int count_of_usable_rows)
        {
        ...
        public double[,] U_matrix(double[,] origin_matrix, double[] eignvalues, double[,] AAt, int count_of_usable_rows)
        {
            ...
            while (i < count_of_usable_rows)
            {
                ...
                double[] vector = Eigenvector(AAt, eignvalues[count_of_eignvalues], u_matrix.GetLength(0));
                for (int j = 0; j < u_matrix.GetLength(0); j++)
                {
                    u_matrix[j, i] = vector[j];
                }
                i++;
                count_of_eignvalues++;
            }
        ...
        public List<double[,]> Decomposition(double[,] matrix, int count_of_usable_rows)
        {
            ...
            double[,] Vt = V_transposed(matrix, eigenvalues, AtA, count_of_usable_rows);
            double[,] S = Sigma(matrix, eigenvalues, count_of_usable_rows);

            double[,] AAt = Matrix_multiplier(matrix, transposed_matrix);
            eigenvalues = Eigenvalues_ascending(AAt);
            
            double[,] U = U_matrix(matrix, eigenvalues, AAt, count_of_usable_rows);
